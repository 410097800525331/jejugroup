...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const [cmsDataModule, apiClientModule] = await Promise.all([
        import('../data/cms-config.js'),
        import('./api_client.js')
    ]);
    const fallbackTabConfig = cmsDataModule.default ?? cmsDataModule.CMS_TAB_CONFIG ?? cmsDataModule.tabConfig ?? {};
    const liveTabConfig = await apiClientModule.fetchAdminPayload('/api/admin/tables/cms').catch((error) => {
        console.warn('[AdminCms] Live config load failed:', error);
        return null;
    });
    const TAB_CONFIG = Object.freeze(liveTabConfig?.tabs ?? fallbackTabConfig);
    const DEFAULT_TAB = liveTabConfig?.defaultTab ?? cmsDataModule.DEFAULT_TAB ?? cmsDataModule.defaultTab ?? 'notices';
