...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const [membersDataModule, apiClientModule] = await Promise.all([
        import('../data/members-config.js'),
        import('./api_client.js')
    ]);
    const fallbackTabConfig = membersDataModule.default ?? membersDataModule.MEMBERS_TAB_CONFIG ?? membersDataModule.tabConfig ?? {};
    const liveTabConfig = await apiClientModule.fetchAdminPayload('/api/admin/tables/members').catch((error) => {
    ...
    const DEFAULT_TAB = liveTabConfig?.defaultTab ?? membersDataModule.DEFAULT_TAB ?? membersDataModule.defaultTab ?? 'member';
