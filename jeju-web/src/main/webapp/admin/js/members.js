 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', () => {
    ...
    const MOCK_CS_RECORDS = Object.freeze([
        { user: '?ㅽ빐??, id: 'haewon@nmixx.com', domain: 'hotel', tier: 'VIP', summary: '?섏쁺???댁슜 ?쒓컙 愿??臾몄쓽', date: '2026-02-20', status: 'WAITING' },
        { user: '?덉쑀吏?, id: 'yujin@ive.com', domain: 'flight', tier: 'REGULAR', summary: '鍮꾪뻾湲??섑븯臾?洹쒖젙 ?덈궡 ?붿껌', date: '2026-02-19', status: 'DONE' },
        { user: '?μ썝??, id: 'wonyoung@ive.com', domain: 'hotel', tier: 'VVIP', summary: '?ㅼ쐞?몃８ ?낃렇?덉씠??媛???щ?', date: '2026-02-19', status: 'WAITING' },
        { user: '源梨꾩썝', id: 'chaewon@lesserafim.com', domain: 'rentcar', tier: 'REGULAR', summary: '?뚰꽣移?諛섎궔 吏?????붽툑??, date: '2026-02-18', status: 'DONE' },
        { user: '移대━??, id: 'karina@aespa.com', domain: 'flight', tier: 'VIP', summary: '醫뚯꽍 蹂寃??ㅻ쪟 而댄뵆?덉씤', date: '2026-02-18', status: 'WAITING' },
        { user: '?ㅼ쑄', id: 'sullyoon@nmixx.com', domain: 'general', tier: 'REGULAR', summary: '?ъ씠??怨꾩젙 ?곕룞 ?ㅻ쪟', date: '2026-02-17', status: 'DONE' }
    ]);
    ...
    const getStatusBadge = (status) => {
        switch(status) {
            case 'WAITING': return '<span class="admin-badge warning">?듬? ?湲?/span>';
            case 'DONE': return '<span class="admin-badge success">泥섎━ ?꾨즺</span>';
    ...
    const getTierBadge = (tier) => {
        switch(tier) {
            ...
            default: return '<span class="admin-badge neutral">?쇰컲</span>';
        }
    };
    ...
    const getDomainBadge = (domain) => {
        switch(domain) {
            ...
            case 'general': return '<span class="admin-badge neutral">怨듯넻</span>';
            default: return '';
        }
    };
    ...
    const renderTable = (domain) => {
        ...
        if (filtered.length === 0) {
            return `<tr><td colspan="7" style="text-align:center; padding: 40px;">?대떦 遺꾨쪟??CS ?묒닔 ?댁뿭???놁뒿?덈떎.</td></tr>`;
        }
        ...
        return filtered.map(item => `
            <tr>
                ...
                <td>
                    <button class="admin-btn admin-btn-primary" style="padding: 4px 8px; font-size: 0.75rem;">?듬? ?ш린</button>
                </td>
            </tr>
        `).join('');
    };
