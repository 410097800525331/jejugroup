(() => {
    ...
    const SECTION_ID = 'members';
    const SECTION_TITLE = '회원 / 권한 / 문의 관리';
    const SHELL_SCRIPT = '../js/admin_shell.js';
    const API_SCRIPT = '../js/api_client.js';
    const CONFIG_SCRIPT = '../data/members-config.js';
    ...
    const ensureRuntime = () => {
        if (window.__ADMIN_MEMBERS_RUNTIME_PROMISE__) {
            return window.__ADMIN_MEMBERS_RUNTIME_PROMISE__;
        }

        window.__ADMIN_MEMBERS_RUNTIME_PROMISE__ = Promise.all([
    ...
    const mountMembers = async ({ root, sectionState }) => {
        if (!root) {
            return () => {};
        }
        ...
        const tableBody = root.querySelector('#members-table-body');
        ...
        const searchButton = root.querySelector('.admin-table-actions .admin-btn-outline');
        const actionButtons = Array.from(root.querySelectorAll('.admin-table-actions .admin-btn'));

        const [configModule] = await Promise.all([ensureRuntime()]);
        if (!isActiveMount()) {
            return () => {};
        }
        const fallbackTabConfig = configModule.default ?? configModule.MEMBERS_TAB_CONFIG ?? configModule.tabConfig ?? {};
        let TAB_CONFIG = Object.freeze(fallbackTabConfig);
        let DEFAULT_TAB = configModule.DEFAULT_TAB ?? configModule.defaultTab ?? 'member';
        ...
        const renderTableBody = (tabKey) => {
            ...
            const rows = (config.rows || []).filter((row) => !keyword || String(row.searchText ?? '').toLowerCase().includes(keyword));
        ...
        void window.AdminApiClient.fetchAdminPayloadInBackground('/api/admin/tables/members', {
            onSuccess: applyLiveTabConfig,
            onError: (error) => {
                if (!isActiveMount()) {
                    return;
                }
                console.warn('[AdminMembers] Live config load failed:', error);
            }
        });
